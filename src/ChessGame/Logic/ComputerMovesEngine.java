        ...
        if(depth == 0){
            ...
            if(computerColor == Piece.Color.WHITE){
                ...
                for(Pair<Coordinate, Coordinate> move : allPossibleMoves){
                    ...
                    bestMoveValue = Math.max(bestMoveValue, checkBestMoveDepth(depth - 1, currentBoard, computerColor.next()));
                    currentBoard.undoMove();
                }
                ...
                for(Pair<Coordinate, Coordinate> move : allPossibleMoves){
                    ...
                    //recursively checking the best board value -- > the next level will get less depth, and opposite color.
                    bestMoveValue = Math.min(bestMoveValue, checkBestMoveDepth(depth - 1, currentBoard, computerColor.next()));
    ...
    private static Pair<Coordinate, Coordinate> ChooseBestMove(Piece.Color computerColor, List<Pair<Coordinate, Coordinate>> allPossibleMoves, Board currentBoard) {
        ...
        if(computerColor == Piece.Color.WHITE){
            ...
            for (Pair<Coordinate, Coordinate> move : allPossibleMoves) {
                ...
                int thisBoardValue = checkBestMoveDepth(4,currentBoard,computerColor);
            ...
            for (Pair<Coordinate, Coordinate> move : allPossibleMoves) {
                currentBoard.movePieceToNewLocation(move.getKey(), move.getValue(), currentBoard.getTileByCoordination(move.getKey()).getCurrentPiece());
                int thisBoardValue = checkBestMoveDepth(6,currentBoard,computerColor);
                if (bestValueBoard > thisBoardValue) {
                    bestMove = move;
                    bestValueBoard = thisBoardValue;
                }
